...
for (const r of DOMAIN_42_REACTIONS) {
...
console.log(`Audited ${DOMAIN_42_REACTIONS.length} reactions for Domain 42:`);
console.log(`Collisions with batches 1-7 & Domains 38-41: ${collisions.length}`);
...
if (collisions.length === 0 && localDups.length === 0 && missingChems.length === 0 && balanceErrors.length === 0) {
  console.log("🌟 DOMAIN 42 IS 100% PERFECT! 100/100 reactions balanced, 0 collisions, 0 duplicates, 0 missing chems!");
} else {
  process.exit(1);
}
